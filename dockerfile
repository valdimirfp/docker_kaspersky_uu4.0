...
# setup working dir
WORKDIR /opt/kuu
RUN wget https://products.s.kaspersky-labs.com/special/kasp_updater3.0/4.1.0.474/english-4.1.0.474/3530393035317c44454c7c31/kuu4.1.0.474_x86_64_en.tar.gz  && tar xvzf kuu4.1.0.474_x86_64_en.tar.gz && rm kuu4.1.0.474_x86_64_en.tar.gz 
...
RUN chmod +x /usr/bin/kavupdater.sh

# add new user
RUN useradd -ms /bin/bash kladmin
RUN chown -R kladmin:kladmin /opt/kuu/

# add folders for updates
RUN mkdir /opt/Updates
RUN mkdir /opt/Temp
RUN mkdir /opt/logs

# add persmittions
RUN chown -R kladmin:kladmin /opt/Updates
